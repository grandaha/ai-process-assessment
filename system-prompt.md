...
`model/*.json` is the single source of truth for every number; `results.json` is derived from it. The deliverable-gate enforces markdown ↔ `results.json` equality. A missing input renders as **PENDING**, never a fabricated number.
