...
- `logo-mark.png` — transparent PNG of the OSL dot-mark (no wordmark), for embedding in generated `.docx` footers (Word can't use the SVG; the wordmark is rendered as live text alongside it). Generated by `gen_logo_mark.py` from the OSL `logo-mark.svg` coordinates — regenerate with `python3 assets/osl/gen_logo_mark.py assets/osl/logo-mark.png`.
- `gen_logo_mark.py` — stdlib (no deps) generator for `logo-mark.png`.
